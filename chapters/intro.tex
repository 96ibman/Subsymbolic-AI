...
A subfield of \linkterm{AI}{def:ai} that posits that intelligence is inherently tied to brains, where information is represented by a simple sequence of pulses that are processed in parallel via simple calculations realized by neurons, and thus concentrates on neural computing.
}{subsymbolic_ai}
...
\item \textit{discrete}, iff the \linkterm{sets}{def:set} of $e$'s state and $a$'s actions are countable; otherwise, \textit{continuous}.
