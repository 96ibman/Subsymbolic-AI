...
\observationbox{
In the limit, the sample average will converge to the true expectation (\linkterm{utility}{utility_rl_passive}) from \Defref{utility_rl_passive}
}
...
\titledbox{Intuition}{
\linkterm{Direct utility estimation}{direct_utility_estimation} searches for $U$ in a \linkterm{hypothesis space}{supervised_learning} that is too large $\leadsto$ many \linkterm{functions}{function} that violate the \linkterm{Bellman Equation}{bellman_equation} $\leadsto$ thus the algorithm often converges slowly
}
