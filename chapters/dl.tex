...
\observationbox{
The "smells" like we need \linkterm{LSTMs}{LSTM}
}
...
Each block represents one \linkterm{LSTM}{LSTM} time step; input are fed successively followed by the token \texttt{<start>} to start the \linkterm{decoder}{seq2seq}
...
\linkterm{Seq2seq}{seq2seq} models were a major breakthrouh in \linkterm{NLP}{NLP} but have the following shortcomings:
\begin{itemize}
...
$a_{ij}$ serves as a weight indicating how much \textbf{focus} the model should place on each part of $s_j$ of teh source sequence when generating the next word.
...
these score are then converted to probabilities using the \linkterm{softmax function}{softmax}, say then we have $[0.73, 0.27]$ for $s_1, s_2$.
...
\problembox{
\linkterm{Greedy decoding}{greedy_decoding} may not always maxmimize the probability of the whole sequence.
}
...
\linkterm{Local beam search}{local_beam_search} is a common choice in machine translation. Conretely:
\begin{itemize}
