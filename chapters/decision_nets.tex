...
\Example{
Consider commuting to work as an example experiment where we have $\Omega = \set{\text{accident on the road}, \text{minor crash on the road}, \text{smooth commute}}$. 
...
\end{itemize}
These are the states our world can be in. We want to define a utility function $F$ representing our happiness. Because $X$ is a \textbf{random}, $F$ becomes \textbf{random} too, we do not get one happiness level, we get a distribution of possible happiness levels based on the underlying reality $\Omega$.
...
\[
\text{MEU}(\emptyset) = \max_a \text{EU}(A=a) = \frac{K}{2}
\]
}
...
\textbf{Exercise : VPI}\hfill
\rule{0.25\textwidth}{0.4pt}%
}

Chef Giordana runs a kitchen that provides food for a large organization. A salad is sold for $\$ 6$ and costs $\$ 4$ to prepare. So a sold salad is a profit of $\$ 2$, and an unsold salad is a loss of $\$ 4$. Actual demand will be $40$ (with probability $0.5$) or $60$ (same probability) each day. 

Each day, Giordana must decide in advance between two options: prepare $40$ or $60$ salads. 

\questionbox{
In the absence of additional information, compute the expected utility of each decision option, and choose the best option
}

We have two actions $A = \set{40, 60}$ and two possible demands $D = \set{40,60}$. We calculate the \linkterm{expected utility}{expected_utility} of each action. 

\begin{align*}
\text{EU}(A = 40) &= P(D = 40) \cdot U(D = 40, A = 40) \\ 
&+ P(D = 60) \cdot U(D = 60, A = 40) \\ 
&= 0.5 \cdot 80 + 0.5 \cdot 80 = 80 \\ 
\text{EU}(A = 60) &= P(D = 40) \cdot U(D = 40, A = 60) \\
&+ P(D = 60) \cdot U(D = 60, A = 60) \\
&= 0.5 \cdot 0 + 0.5 \cdot 120 = 60
\end{align*}

\answerbox{
The action that \linkterm{maximizes expected utility}{def:MEU} without any additional information is to prepare $40$ salads, with:

\[
\text{MEU}(\emptyset) = \max_a \text{EU}(A=a) = 80
\]
}

\questionbox{
She is considering a new ordering system, where she knows the demand perfectly in advance. So she can always choose the better of the two options. How much she is willing to pay for such system that provides perfect information?
}

We need to compute the maximum expected utility with perfect information and subtract from it the one without any information ($80$).

\[
\text{VPI}(E) = \text{MEU}(A \mid E) - \text{MEU}(\emptyset)
\]

What is the maximum expected utility given that we know what the demand is? Well, the demand will be either $40$ or $60$, and if it is $40$ we will take the action $A=40$ and the same goes for if the demand is $60$. So we have:

\begin{align*}
\text{VPI}(E) &= \left(P(D=60) \cdot U(A=60, D=60) + P(D=40) \cdot U(A=40, D=40)\right) - 80 \\
&= \left( 0.5 \cdot 120 + 0.5 \cdot 80 \right) - 80 \\
&= \left( 60 + 40 \right) - 80 \\
&= 100  - 80 = \$20
\end{align*}

\answerbox{
We are willing to pay $\$ 20$ for such perfect information about the demand.
}
...
\begin{enumerate}
    \item The system tells us demand is $40$:
    \begin{itemize}
        \item without information, we already make $40$ salads and make $\$80$ profit
        \item with this information, nothing changes, we will make the $40$ salads and get the $\$80$ profit. Hence the value of this information will be $80 - 80 = 0$
    \end{itemize}
    \item The system tells us demand is $60$:
    \begin{itemize}
        \item without information, we are making $40$ salads and make $\$80$ profit.
        \item with this information, we will make $60$ and profit $\$120$
        \item Hence the value of this information in this scenario is $120 - 80 = \$40$.
    \end{itemize}
\end{enumerate}


Since we cannot predict which scenario will happen tomorrow, we take the expected value of these two outcomes across our \linkterm{prior}{def:bayesian_terms} uncertainty:
\[
\text{VPI} = P(\text{scenario 1}) \cdot 0 + P(\text{scenario 2}) \cdot 40 = 0.5 \cdot 0 + 0.5 \cdot 40 = \$20
\]
