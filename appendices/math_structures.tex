\subsection{Algebraic Structures}\label{sec:math_structure}
\definition{Mathematical (Algebraic) Structure}{
...
An abelian group (or commutative group) is a \linkterm{group}{group}
$\tuple{G,\circ,e,\inverse{.}}$ such that:
\[
...
\definition{Field}{
A field is a \linkterm{mathematical structure}{def:math_structure}
\[
\tuple{F,+,0,-,*,1}
\]
consisting of:
\begin{itemize}
    \item A base \linkterm{set}{def:set} $F$,
    \item An addition operation such that $\tuple{F,+,0,-}$ is an abelian \linkterm{group}{group},
    \item A multiplication operation such that
    \[
    \tuple{F \setminus \set{0}, *, 1, a \mapsto \frac{1}{a}}
    \]
    is an abelian \linkterm{group}{group},
    \item Distributivity:
    \[
    a*(b+c)=(a*b)+(a*c)
    \]
...
\textbf{Examples:}
\begin{itemize}
    \item $\mathbb{Q}$ (rational numbers) is a \linkterm{field}{field}.
    Every non-zero rational number has a multiplicative inverse in $\mathbb{Q}$.
    For example:
    \[
    2^{-1} = \frac{1}{2}
    \]

    \item $\mathbb{R}$ (real numbers) is a \linkterm{field}{field}.

    \item $\mathbb{C}$ (complex numbers) is a \linkterm{field}{field}.

    \item $\mathbb{Z}$ (integers) is \textbf{not} a \linkterm{field}{field},
    because most integers do not have multiplicative inverses in $\mathbb{Z}$.
    For example:
    \[
    \frac{1}{2} \notin \mathbb{Z}
    \]
    ...
    \mathbb{R}^{n \times n}
    \]
    is generally \textbf{not} a \linkterm{field}{field},
    because matrix multiplication is not commutative and some non-zero
