...
\noindent\makebox[\textwidth]{%
\rule{0.25\textwidth}{0.4pt}\hfill
\textbf{Exercise : Discounting}\hfill
\rule{0.25\textwidth}{0.4pt}%
}
...
\noindent\makebox[\textwidth]{%
\rule{0.25\textwidth}{0.4pt}\hfill
\textbf{Excursion : Recall - Geometric Series}\hfill
\rule{0.25\textwidth}{0.4pt}%
}
...
\begin{align*}
   S_k &=  a + ar + ar^2 + \dots + ar^{k-1} \\ 
   r \cdot S_k &=  ar + ar^2 + ar^3 + \dots + ar^{k}
\end{align*}

Subtract ($S_k - rS_k$)
\[
S_k - rS_k =  (a + ar + ar^2 + \dots + ar^{k-1}) - (ar + ar^2 + ar^3 + \dots + ar^{k})
\]

\redbox{
Every middle term cancels, leaving only the first term from the first sum and the last term from the second sum:
}
...
Factor out $S_k$ on the left side and $a$ on the right side:
\[
S_k (1 - r) = a (1-r^k)
\]

Divide by $(1 - r)$ (valid as long as $r \neq 1$):

\[ S_k = a \left( \frac{1-r^k}{1-r} \right) \quad \text{for } r \neq 1 \]

\redbox{
This allows the calculation of the sum of the first $k$ terms instantly, without adding them individually.
}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{images/mdp_exercise.png}
\end{figure}

\begin{itemize}
    \item The world is $101$ fields wide 
    \begin{itemize}
        \item We have $101$ top and $101$ bottom, that makes $202$
        \item We also have the \textit{Start} state, we have $203$ total fields
    \end{itemize}
    \item In the \textit{Start} state, an agent has two possible actions, \texttt{up} and \texttt{down}.
    \item It can't return to \textit{Start} and can't pass any gray fields.
    \item After the first move, the only possible action is \texttt{right}.
\end{itemize}

\questionbox{
Model this world as a \linkterm{MDP}{MDP}.
}

We model states as integers. e.g. the start state is $0$ and then we have $1,\cdots 101$ representing the top fields and $-1, \cdots -101$ representing the bottom fields. 

\begin{align*}
\mathcal{S} &:= \set{-101, \cdots, -1, 0, 1, \cdots 101} \\ 
\mathcal{I} &:= \set{0}
\end{align*}

For actions, we have:
\begin{align*}
A(0) &= \set{\texttt{up}, \texttt{down}} \\
A(s) &= \set{\texttt{right}} \quad \forall s \in \mathcal{S} \setminus \set{0}
\end{align*}

We define the \linkterm{reward function}{MDP}:
\begin{align*}
R(0) &= 0 \qquad R(1) = 50 \qquad R(-1)=-50 \\ 
R(s) &= -1 \qquad \forall s \in \set{2,3, \cdots 101} \\
R(s) &= +1 \qquad \forall s \in \set{-2,-3, \cdots -101} \\
\end{align*}

We have a \linkterm{deterministic}{env_types} \linkterm{transition model}{transition_model_hmm}:

\textbf{Initial State $s=0$}:

\begin{align*}
P(1 \mid 0, \texttt{up}) &= 1 \qquad P(1 \mid 0, \texttt{down}) = 0 \\ 
P(-1 \mid 0, \texttt{up}) &= 0 \qquad P(-1 \mid 0, \texttt{down}) = 1 \\ 
P(s' \mid 0,a) &= 0 \qquad \forall s' \in \mathcal{S} \setminus \set{-1,0,1}, \quad a \in \set{\texttt{up}, \texttt{down}}
\end{align*}

\textbf{Other States $s\neq 0$}:

\begin{align*}
P(s+1 \mid s, \texttt{right}) &= 1 \qquad \forall s \in \set{1, \cdots 100} \\ 
P(s-1 \mid s, \texttt{right}) &= 1 \qquad \forall s \in \set{-1, \cdots -100} \\
P(101 \mid 101, \texttt{right}) &= P(-101 \mid -101, \texttt{right}) = 1 \qquad \text{(terminal states)} \\ 
\end{align*}
...
\questionbox{
For what \linkterm{discount factors}{discounted_rewards} $\gamma$ should the agent choose \texttt{up} and for which \texttt{down}?
}

If the agent chooses \texttt{up}. We compute $U([1, \cdots 101])$:
\begin{align*}
U([1, \cdots 101]) &= 50 + \gamma (-1) + \gamma^2 (-1) + \gamma^3 (-1) + \cdots + \gamma^{100} (-1) \\
U([1, \cdots 101]) &= 50 - \sum_{i=1}^{100} \gamma^i
\end{align*}

If the agent chooses \texttt{down}. We compute $U([-1, \cdots -101])$:
\begin{align*}
U([-1, \cdots -101]) &= -50 + \gamma (1) + \gamma^2 (1) + \gamma^3 (1) + \cdots + \gamma^{100} (1) \\
U([-1, \cdots -101]) &= -50 + \sum_{i=1}^{100} \gamma^i
\end{align*}
...
\redbox{
In order to know for what \linkterm{discount factor}{discounted_rewards} should the agent choose \texttt{up/down}. We need first to compute the \linkterm{discount factor}{discounted_rewards} that make the agent indifferent.
}

The agent will be indifferent when
\[
U([1, \cdots 101]) = U([-1, \cdots -101])
\]

i.e. when:
\begin{align*}
50 - \sum_{i=1}^{100} \gamma^i &= -50 + \sum_{i=1}^{100} \gamma^i \\
50 + 50 &=  \sum_{i=1}^{100} \gamma^i + \sum_{i=1}^{100} \gamma^i \\
100 &=  2 \cdot \sum_{i=1}^{100} \gamma^i \\
50 &= \sum_{i=1}^{100} \gamma^i \\ 
\gamma &\approx 0.98 \qquad \text{(solve for $\gamma$)}
\end{align*}

\commandnote{
Solving such equations is beyond the scope of this course. Here is one way to compute this with Python:
}

\begin{codeblocklst}[language=Python, numbers=left, numberstyle=\tiny\color{gray}]
import scipy.optimize as opt

def f(gamma):
    return sum(gamma**i for i in range(1, 101)) - 50

sol = opt.root_scalar(f, bracket=[0.001, 0.999])
print("Root:", sol.root)
\end{codeblocklst}

This will output \inlinecode{Root: 0.9843976692056645}

Therefore, the agent will choose \texttt{up} if $\gamma < 0.98$. When $\gamma$ is smaller, the agent places less emphasis on future rewards. When $\gamma$ is larger, the agent places more emphasis on future rewards.

\questionbox{
What is the \linkterm{optimal policy}{optimal_policy_mdp} if the upper path is better?
}

\begin{align*}
\pi^{*}(0) &= \texttt{up} \\ 
\pi^{*}(s) &= \texttt{right} \qquad \forall s \in \mathcal{S} \setminus \set{0}
\end{align*}
...
\noindent\makebox[\textwidth]{%
\rule{0.25\textwidth}{0.4pt}\hfill
\textbf{Excursion : The story of approximating $\gamma$}\hfill
\rule{0.25\textwidth}{0.4pt}%
}
...
Let's expand the summation $\sum_{i=1}^{100} \gamma^i$ by plugging in $i = 1, 2, 3, \dots, 100$:
\[
\sum_{i=1}^{100} \gamma^i = \gamma^1 + \gamma^2 + \gamma^3 + \dots + \gamma^{100}
\]

This can be rewritten as:
\[
\gamma^1 + \gamma^2 + \gamma^3 + \dots + \gamma^{100} = \gamma \cdot \left( 1 + \gamma^1 + \gamma^2 + \dots + \gamma^{99} \right)
\]

Notice that this is exactly:
\[
\gamma \cdot \left( \gamma^0 + \gamma^1 + \gamma^2 + \dots + \gamma^{99} \right)
\]

Now, the terms inside the parentheses perfectly match the definition of a $k$-th partial sum of a geometric series (\Defref{geometric_series}).
...
Closed form of the $k$-th partial sum:
\[ S_k = a \left( \frac{1-r^k}{1-r} \right) \quad \text{for } r \neq 1 \]
}

\commandnote{
In our case, $a = 1$,  $r = \gamma$,  and $k = 100$ $\leadsto$ the term inside the parentheses is $S_{100}$
}

We write in its closed form:
\[
S_{100} = 1 \cdot \left( \frac{1-\gamma^{100}}{1-\gamma} \right)
\]

Back to what we had:
\begin{align*}
    \sum_{i=1}^{100} \gamma^i &= \gamma \cdot S_{100} \\ 
    \sum_{i=1}^{100} \gamma^i &= \gamma \cdot \left( \frac{1-\gamma^{100}}{1-\gamma} \right) = \frac{\gamma(1 - \gamma^{100})}{1 - \gamma}
\end{align*}

Now, substitute this closed-form expression back into the original equation:
\begin{align*}
50 &= \frac{\gamma(1 - \gamma^{100})}{1 - \gamma} \\ 
50(1 - \gamma) &= \gamma(1 - \gamma^{100}) \\ 
50 - 50\gamma &= \gamma - \gamma^{101} \\ 
\gamma^{101} - 50\gamma - \gamma + 50 &= 0 \\ 
\gamma^{101} - 51\gamma + 50 &= 0
\end{align*}

We are left with a polynomial equation of degree 101.

\commandnote{
\begin{itemize}
    \item There is no general algebraic formula to solve polynomial equations of degree 5 or higher. \hfill (Abel-Ruffini Theorem)
    \item We rely on \textbf{numerical methods} to \textit{approximate} the root.
    \item To find the exact value, root-finding algorithms (like the Newton-Raphson method used by the Python script) convert the equation into a function: $f(\gamma) = \gamma^{101} - 51\gamma + 50$
\end{itemize}
}

The algorithm looks for the value of $\gamma$ where $f(\gamma) = 0$. It does this by making a guess, evaluating the function, and using the calculus derivative of the function to systematically narrow down the error.

The first guess comes from assuming we have the full infinite series and assuming $\abs{\gamma} < 1$. If the agent's horizon didn't stop at 100 steps but instead went on forever ($k \to \infty$), the definition states that because $\vert{}\gamma\vert{} < 1$, the term $\gamma^{k}$ would shrink toward $0$:

$$\lim_{k \to \infty} \gamma^k = 0$$

If $\gamma^{100} \approx 0$ is substituted into the formula, the equation simplifies to:
\begin{align*}
    50 &\approx \frac{\gamma(1 - 0)}{1 - \gamma} \\ 
    50 &\approx \frac{\gamma}{1 - \gamma}
\end{align*}

This can be solved easily using standard algebra:
\begin{align*}
    50(1 - \gamma) &\approx \gamma \\ 
    50 - 50\gamma &\approx \gamma \\ 
    50 &\approx 51\gamma \\ 
    \gamma &\approx \frac{50}{51} \approx 0.9804
\end{align*}

Using the derivative $f'(\gamma) = 101\gamma^{100} - 51$, the algorithm takes a starting guess (like $\gamma_0 = 0.98$) and refines it iteratively:

$$\gamma_{new} = \gamma_{old} - \frac{f(\gamma_{old})}{f'(\gamma_{old})}$$

Our update rule is:

$$\gamma_{n+1} = \gamma_n - \frac{f(\gamma_n)}{f'(\gamma_n)}$$

Iteration 1: Starting with $\gamma_0 = 0.98$

\begin{align*}
f(0.98) &= (0.98)^{101} - 51(0.98) + 50 \\ 
f(0.98) &\approx 0.12933 - 49.98 + 50 = 0.14933 \qquad (0.98^{101} \approx 0.12933) \\
\end{align*}

Next, we calculate the value of the derivative at our initial guess. The derivative represents the slope of the tangent line at this point:

\begin{align*}
    f'(0.98) &= 101(0.98)^{100} - 51 \\ 
    f'(0.98) &\approx 101(0.13197) - 51 = 13.32897 - 51 = -37.67103 \qquad (0.98^{100} \approx 0.13197)
\end{align*}

Now, we plug these two values into the update rule to find our next guess, $\gamma_1$
\begin{align*}
    \gamma_1 &= \gamma_0 - \frac{f(\gamma_0)}{f'(\gamma_0)} \\ 
    \gamma_1 &= 0.98 - \frac{0.14933}{-37.67103} \\ 
    \gamma_1 &\approx 0.98 - (-0.003964) = 0.983964
\end{align*}

\redbox{
If we keep going, the values rapidly stabilize. We stop when the function value $f(\gamma_n)$ is zero (or close enough to it), which means we have found our root.
}
...
\toprule
\textbf{Iteration ($n$)} & \multicolumn{2}{c}{\textbf{Guess ($\gamma_n$)}} & \multicolumn{2}{c}{\textbf{Function Value $f(\gamma_n$)}} \\
\midrule
0 & 0&98     & 0&14933 \\
1 & 0&983964 & 0&01423 \\
2 & 0&984425 & 0&00014 \\
